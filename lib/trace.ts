import { ElectrumSession } from "./electrum"
...
function calcCpObscurity(intermediates: AddressInfo[]): number {
  if (intermediates.length === 0) return 0
  const avg =
    intermediates.reduce((s, a) => s + a.historyLength, 0) / intermediates.length
  if (avg <= 1) return 0
  if (avg >= 10) return 1
  return (avg - 1) / 9
}
...
function buildCexLink(
  ...
  const intermediateInfos = pathAddrs
    .slice(1, -1)
    .map((a) => addrInfo.get(a) ?? { historyLength: 0, parentAddr: null })
  ...
  const pathWallets: IntermediateWallet[] = pathAddrs.slice(1).map((addr) => {
    ...
    return {
      ...
      directness:
        flag === "CEX" && addr === pathAddrs[pathAddrs.length - 1]
          ? `${exchange} — confirmed CEX`
          : directnessLabel(histLen),
...
function recordCex(
  ...
  links.push(buildCexLink(path, addrInfo, exchange, flag))
}
...
async function traceSource(
  ...
  for (let depth = 0; depth <= maxDepth && currentLevel.length > 0; depth++) {
    ...
    for (let i = 0; i < currentLevel.length; i++) {
      ...
      if (res.error) {
        ...
        if (depth > 0) {
          ...
          recordCex(addr, parentMap.get(addr)!, "Unknown (huge history)", "possible CEX",
            sourceAddr, parentMap, addrInfo, links, visited, 999999)
        } else {
      ...
      if (depth > 0 && isPossibleCex(history.length)) {
        ...
        recordCex(addr, parentMap.get(addr)!, "Unknown (high activity)", "possible CEX",
          sourceAddr, parentMap, addrInfo, links, visited, history.length)
        continue
      }
    ...
    for (const { addr, txHashes } of toProcess) {
      ...
      for (const tx of txs) {
        for (const outAddr of extractOutputAddresses(tx)) {
          if (outAddr === addr || visited.has(outAddr)) continue

          const exch = checkCex(outAddr, cexDb)
          if (exch) {
            recordCex(outAddr, addr, exch, "CEX", sourceAddr, parentMap, addrInfo, links, visited)
            onProgress(`    CEX FOUND: ${exch} — ${outAddr.slice(0, 12)}… (${reconstructPath(parentMap, sourceAddr, outAddr).length - 1} hops)`)
    ...
    // --- Step 4: depth 0 only — batch resolve inputs for CEX checking ---
    if (depth === 0) {
      ...
      if (inputFetch.length > 0) {
        onProgress(`  depth 0: resolving ${inputFetch.length} input txs`)
      ...
      for (const { addr, txHashes } of toProcess) {
        ...
        for (const tx of txs) {
          for (const vin of tx.vin) {
            ...
            if (exch) {
              recordCex(inAddr, addr, exch, "CEX", sourceAddr, parentMap, addrInfo, links, visited)
              onProgress(`    CEX FOUND (input): ${exch} — ${inAddr.slice(0, 12)}… (1 hop)`)
            }
          }
        }
      }
    ...
    if (depth < maxDepth && allCandidates.size > 0) {
      ...
      for (let i = 0; i < candidates.length; i++) {
        ...
        if (isPossibleCex(hLen)) {
          ...
          recordCex(cAddr, cParent, "Unknown (high activity)", "possible CEX",
            sourceAddr, parentMap, addrInfo, links, visited, hLen)
      ...
      onProgress(`  depth ${depth}: ${allCandidates.size} candidates → ${nextLevel.length} queued${skipped > 0 ? ` (${skipped} skipped)` : ""} for depth ${depth + 1}`)
    }
...
export async function runRealTrace(
  ...
  const session = new ElectrumSession(host, port, SESSION_TIMEOUT, useTls)
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    if (res.status === "fulfilled") {
      ...
      resultSources.push({ address: src.address, derivationPath: src.derivationPath, balanceBtc: src.balanceBtc, txCount: src.txCount, links: [] })
    }
  }
